...
export default function AboutPage() {
  ...
  return (
    <>
      ...
      {/* ─── JOURNEY ─── */}
      <section style={{ padding: "110px 2rem 130px", background: PRIMARY_PALE, position: "relative", overflow: "hidden" }}>
        <div style={{ maxWidth: 1280, margin: "0 auto", position: "relative", zIndex: 1 }}>
          <FadeIn>
            <div style={{ textAlign: "center", marginBottom: 80 }}>
              <div className="fp" style={{ color: PRIMARY, fontSize: 11, letterSpacing: 5, textTransform: "uppercase", fontWeight: 600, marginBottom: 16 }}>Our Milestones</div>
              <h2 className="fg" style={{ fontSize: 54, color: PRIMARY, fontWeight: 300, lineHeight: 1.1 }}>The <em>Journey</em> of TaxAmenity India</h2>
            </div>
          </FadeIn>
          <div style={{ position: "relative" }}>
            <div style={{ position: "absolute", top: "50%", left: "4%", right: "4%", height: 3, background: `linear-gradient(to right, transparent, ${PRIMARY}60, ${PRIMARY}, ${PRIMARY}60, transparent)`, transform: "translateY(-50%)", borderRadius: 2 }} />
            <div style={{ display: "grid", gridTemplateColumns: `repeat(${journey.length}, 1fr)`, gap: 16, position: "relative" }}>
              {journey.map((item, i) => {
                const isTop = i % 2 === 0;
                return (
                  <FadeIn key={item.year} delay={i * 0.1} dir={isTop ? "down" : "up"}>
                    <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
                      {isTop && (
                        <div style={{ background: "#fff", borderRadius: 16, padding: "22px 18px", marginBottom: 20, textAlign: "center", boxShadow: `0 8px 32px ${PRIMARY}10`, border: `1px solid ${PRIMARY}12`, cursor: "default", transition: "transform 0.3s, box-shadow 0.3s", width: "100%" }}
                          onMouseOver={e => { e.currentTarget.style.transform = "translateY(-6px)"; e.currentTarget.style.boxShadow = `0 20px 50px ${PRIMARY}20`; }}
                          onMouseOut={e => { e.currentTarget.style.transform = "none"; e.currentTarget.style.boxShadow = `0 8px 32px ${PRIMARY}10`; }}>
                          <div className="fg" style={{ fontSize: 26, color: PRIMARY, fontWeight: 700, marginBottom: 4 }}>{item.year}</div>
                          <div className="fg" style={{ fontSize: 18, color: PRIMARY, fontWeight: 600, marginBottom: 8 }}>{item.title}</div>
                          <p className="fp" style={{ fontSize: 11, color: "#6b7a8a", lineHeight: 1.65, fontWeight: 300 }}>{item.desc}</p>
                        </div>
                      )}
                      {!isTop && <div style={{ height: 180 }} />}
                      <div style={{ width: 52, height: 52, borderRadius: "50%", background: i % 2 === 0 ? PRIMARY : "#fff", border: `3px solid ${PRIMARY}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22, zIndex: 2, boxShadow: `0 0 0 6px ${PRIMARY_PALE}, 0 8px 20px ${PRIMARY}30`, flexShrink: 0 }}>
                        {item.icon}
                      </div>
                      {!isTop && (
                        <div style={{ background: PRIMARY, borderRadius: 16, padding: "22px 18px", marginTop: 20, textAlign: "center", boxShadow: `0 8px 32px ${PRIMARY}30`, width: "100%", cursor: "default", transition: "transform 0.3s" }}
                          onMouseOver={e => e.currentTarget.style.transform = "translateY(6px)"}
                          onMouseOut={e => e.currentTarget.style.transform = "none"}>
                          <div className="fg" style={{ fontSize: 26, color: "#fff", fontWeight: 700, marginBottom: 4 }}>{item.year}</div>
                          <div className="fg" style={{ fontSize: 18, color: "rgba(255,255,255,0.9)", fontWeight: 600, marginBottom: 8 }}>{item.title}</div>
                          <p className="fp" style={{ fontSize: 11, color: "rgba(255,255,255,0.6)", lineHeight: 1.65, fontWeight: 300 }}>{item.desc}</p>
                        </div>
                      )}
                      {isTop && <div style={{ height: 180 }} />}
                    </div>
